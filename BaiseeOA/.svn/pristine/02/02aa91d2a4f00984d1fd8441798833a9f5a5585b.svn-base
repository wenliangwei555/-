...
var PopupLayer = new Class({
	...
	_loadOverlay:function(){                //加载遮罩
		pageWidth = ($.support.opacity=="6.0")?$(document).width()-21:$(document).width();
	...
	doresize:function(){
		this.overlay?this.overlay.css({width:($.support.opacity=="6.0")?$(document).width()-21:$(document).width(),height:($.support.opacity=="6.0")?$(document).height()-4:$(document).height()}):null;
		if(this.isSetPosition){
			this.setPosition(this.trigger.offset().left + this.options.offsets.x, this.trigger.offset().top + this.trigger.get(0).offsetHeight + this.options.offsets.y);
		}
	},
